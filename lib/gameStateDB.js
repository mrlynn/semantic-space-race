...
export class GameState {
  ...
  hitVectorGem(gemId, playerId) {
    ...
    // Check if expired (30 seconds + 2 second grace period for network latency)
    const now = Date.now();
    const age = now - gem.spawnTime;
    if (age > 32000) { // 30 seconds + 2 second grace period
      console.log(`💎 [GameState] Gem expired: age=${age}ms, spawnTime=${gem.spawnTime}, now=${now}`);
  ...
  hitBadAsteroid(asteroidId, playerId) {
    ...
    if (age > 32000) { // 30 seconds + 2 second grace period
      console.log(`☄️ [GameState] Asteroid expired: age=${age}ms, spawnTime=${asteroid.spawnTime}, now=${now}`);
